...
import com.dmw.noble.adaptor.InsuredAdapter;
...
import com.dmw.noble.model_health.v2.InsuredMember;
import com.dmw.noble.utils.AppUtils;

import java.util.ArrayList;
import java.util.Objects;

public class HealthReviewActivity extends AbstractActivity
    ...
    String nomineeName, nomineeRelation, nomineeMobile, quotationId, netPremium, serviceTax,
            grossPremium, planName, planType, imgPath, companyName;

    ArrayList<String> proposerFirstName, proposerLastName, proposerAddress1, proposerAddress2,
            proposerAddress3, proposerMobile, firstName, lastName, gender, dob, bajajMobile, bd1,
            bd2, bd3, bajajNF, bajajNL, bajajNR, sbiMobile, sbiAddress;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mBundle != null) {
            ...
            bajajMobile = mBundle.getStringArrayList(AppUtils.HL_BAJAJ_PHONE);
            bd1 = mBundle.getStringArrayList(AppUtils.HL_BAJAJ_A1);
            bd2 = mBundle.getStringArrayList(AppUtils.HL_BAJAJ_A2);
            bd3 = mBundle.getStringArrayList(AppUtils.HL_BAJAJ_A3);
            ...
            sbiAddress = mBundle.getStringArrayList(AppUtils.HL_SBI_ADDRESS1);

            if (firstName.size() > 0) {

                for (int sq = 0; sq < firstName.size(); sq++) {
                    InsuredMember insuredMember = new InsuredMember();
                    insuredMember.setName(firstName.get(sq) + " " + lastName.get(sq));
        ...
        if (!companyName.equalsIgnoreCase("bajaj")
            ...
            edtProposerName.setText(String.format("%s %s", proposerFirstName.get(0), proposerLastName.get(0)));
            edtProposerMobile.setText(proposerMobile.get(0));
            edtProposerAddress.setText(String.format("%s %s %s", proposerAddress1.get(0),
                    proposerAddress2.get(0), proposerAddress3.get(0)));
        } else if (companyName.equalsIgnoreCase("bajaj")) {
            edtProposerName.setText(String.format("%s %s", firstName.get(0), lastName.get(0)));
            edtProposerMobile.setText(bajajMobile.get(0));
            edtProposerAddress.setText(String.format("%s %s %s", bd1.get(0),
                    bd2.get(0), bd3.get(0)));
            edtNomineeName.setText(bajajNF.get(0) + " " + bajajNL.get(0));
            edtNomineeMobile.setText(bajajMobile.get(0));
            edtNomineeRelation.setText(bajajNR.get(0));
        } else if (companyName.equalsIgnoreCase("sbi")) {
            edtProposerName.setText(String.format("%s %s", firstName.get(0), lastName.get(0)));
            edtProposerMobile.setText(sbiMobile.get(0));
            edtProposerAddress.setText(sbiAddress.get(0));
        }
    ...
    public void onResponse(Object response) {
        if (response instanceof Session) {
            ...
            //else Toast.makeText(mContext, "" + cResponse.getMsg(), Toast.LENGTH_SHORT).show();
        }
        progressdialog.dismiss();
    }


    public void onShareLinkHealthData(View view) {
        try {
