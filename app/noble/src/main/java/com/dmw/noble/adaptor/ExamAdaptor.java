...
public class ExamAdaptor extends BaseAdapter {
    ...
    public ExamAdaptor(Context mContext, ArrayList<QuestionDatum> questionData) {
        ...
        if (mContext instanceof OnItemTapListener) {
            onItemTapListener = (OnItemTapListener) mContext;
        } if (mContext instanceof OnTotalNumber) {
            onTotalNumber = (OnTotalNumber) mContext;
        }
    }
